...
- (void)reloadData
{
    ...
    if (self.bubbleDataSource && (count = [self.bubbleDataSource numberOfRowsForBubbleTable:self]) > 0)
    {
        ...
        for (int i = 0; i < count; i++)
        {
            BTVBubbleData *data = (BTVBubbleData *)[_bubbleData objectAtIndex:i];
...
- (void)tableviewGestureRecognizerTapped:(UITapGestureRecognizer *)gestureRecognizer
{
    ...
    BTVBubbleTableViewCell *cell = (BTVBubbleTableViewCell *)[self cellForRowAtIndexPath:indexPath];
    
    if ([self.bubbleDelegate respondsToSelector:@selector(bubbleTableView:didSelectBubbleAtIndex:)])
    {
        if ([cell isKindOfClass:[BTVBubbleTableViewCell class]] && indexPath)
        {
            ...
            {
                NSUInteger index = [_bubbleData indexOfObject:cell.data];
